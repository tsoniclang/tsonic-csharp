...
export function finalizeCsharpNumericRestSequences(
  context: CsharpSourceProfileCallPolicyContext,
  call: CsharpSelectedTargetCall,
): CsharpSelectedTargetCall | undefined {
  if (!call.targetMember.parameters.some(parameter => parameter.csharpSequenceHolePolicy !== undefined)) return call;
  ...
  for (const [sourceArgumentIndex, argument] of context.source.sourceArguments.entries()) {
    ...
    if (first === undefined && sourceType?.kind === "tuple" && sourceType.elements.length === 0 &&
      ...
      context.source.sourceSelectedSignatureParameters[0]?.rest === true &&
      call.targetMember.parameters.length === 1 && call.targetMember.parameters[0]?.paramsArray === true) {
      first = { sourceArgumentIndex, effectiveArgumentIndex: sourceArgumentIndex,
        sourceForm: "spread-sequence", targetParameterIndex: 0, targetParameter: call.targetMember.parameters[0] };
    ...
    if (parameter.csharpSequenceHolePolicy !== "number-nan") continue;
    if (parameter.paramsArray !== true || parameter.type.kind !== "array" ||
      !targetTypeRefEquals(parameter.type.element, csharpSourcePrimitiveTargetType("float64")) ||
    ...
    if (elements.some(({conversion}) => conversion.kind !== "identity" &&
      (conversion.kind !== "implicit" || conversion.proof !== "numeric"))) return undefined;
    ...
    sequences.push(Object.freeze({
      sourceArgumentIndex, expression, sourceType, elements: Object.freeze(elements),
