import type { Type, TypeAliasApplicationInfo } from "@tsonic/tsts";
...
export function resolveCsharpConditionalApplication(
  ...
  for (const step of application.conditionalSteps) {
    if (step.branch === "deferred") {
      const declaration = scope.host.ast.parent(step.conditional);
      if (declaration === undefined || !scope.host.ast.is.IsTypeAliasDeclaration(declaration) ||
    ...
    for (const binding of step.bindings) {
      ...
      const targetType = index < 0 ? scope.resolveDirectSourceFacts(queries.facts.typeSubjects(sourceType), queries.sourceFile, nextState(bound)) ??
        (queries.types.isNumberLike(sourceType) || queries.types.isBigIntLike(sourceType) ? undefined
          : scope.resolveTypeWithState(sourceType, queries.sourceFile, nextState(bound))) : arguments_[index];
...
export function resolveCsharpSemanticConditionalType(
  ...
  if (selectedSources.some((argument, index) => argument !== application!.bindings[index]!.argument)) {
    const conditional = application.conditionalSteps[0]?.conditional;
    const declaration = conditional === undefined ? undefined : scope.host.ast.parent(conditional);
    if (declaration === undefined || !scope.host.ast.is.IsTypeAliasDeclaration(declaration)) return { type: undefined };
