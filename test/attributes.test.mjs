...
function fakeInput(sourceFile, options) {
  return {
    ast: {
      ...
      modifiers: (candidate) => candidate?.Modifiers?.Nodes ?? [],
      modifierFlags: (candidate) => candidate?.ModifierFlags ?? 0,
      hasModifier: (candidate, flags) => ((candidate?.ModifierFlags ?? 0) & flags) !== 0,
