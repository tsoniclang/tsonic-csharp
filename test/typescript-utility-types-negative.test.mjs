...
test("every pinned TypeScript utility preserves its negative source contract before C# lowering", () => {
  const checked = checkCsharpSource({ sourceText: invalidUtilitySource });

  assert.deepEqual(checked.extensionDiagnostics, []);
  assert.equal(checked.source.diagnostics.length, 39, checked.sourceDiagnosticsText);
  for (const expected of expectedDiagnostics) {
    assert.match(checked.sourceDiagnosticsText, expected);
  }
});
