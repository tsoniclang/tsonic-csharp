...
test("explicit safety reaches the planner through target-owned policy only", () => {
  const explicitSafety = product("src/backend/planner/explicit-safety.ts");
  const nativePointers = product(
    "src/backend/planner/expression-native-pointers.ts",
  );
...
test("C# unsafe permission uses typed syntax traversal", () => {
  const permission = product("src/backend/planner/unsafe-requires.ts");
  const members = product("src/backend/planner/unsafe-members.ts");
  const expressions = product("src/backend/planner/unsafe-expressions.ts");
...
test("C# never restores compilation-wide or containing-type unsafe inference", () => {
  const marking = product("src/backend/planner/unsafe-marking.ts");
...
test("C# language, memory rules, and unsafe permission remain separate controls", () => {
  ...
  const properties = product(
    "src/backend/planner/project-property-options.ts",
  );
  const safety = product("src/backend/planner/explicit-safety.ts");
